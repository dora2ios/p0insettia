...
uint32_t find_mount_union(uint32_t region, uint8_t* kdata, size_t ksize)
{
    const struct find_search_mask search_masks[] =
    {
        {0xFF00, 0xD100}, // bne.n
    ...
    return ((uintptr_t)fn_start) + 2 - ((uintptr_t)kdata);
}
...
uint32_t find_amfi_cred_label_update_execve(uint32_t region, uint8_t* kdata, size_t ksize)
{
    ...
    printf("%x\n", (uintptr_t)hook_execve-(uintptr_t)kdata);
...
uint32_t find_amfi_vnode_check_signature(uint32_t region, uint8_t* kdata, size_t ksize)
{
    ...
    uint8_t* point = memmem(kdata, ksize, str, sizeof(str));
    if(!point)
        return 0;
...
uint32_t find_sbops(uint32_t region, /*uint32_t kextbase,*/ uint8_t* kdata, size_t ksize)
{
    uint32_t sbPolicyFullName = (uint32_t)memmem(kdata, ksize, "Seatbelt sandbox policy", strlen("Seatbelt sandbox policy")) - (uintptr_t)kdata;
    
    uint32_t search[1];
    search[0] = sbPolicyFullName/*+kextbase*/;
    
    uint32_t policyConf_mpcName = (uint32_t)memmem(kdata, ksize, &search, 4) - (uintptr_t)kdata - 4;
    
    uint32_t sb_mpcOps = *(uint32_t*)(kdata + (policyConf_mpcName + 0x10));
    
    uint32_t sbops = sb_mpcOps/*-kextbase*/;
